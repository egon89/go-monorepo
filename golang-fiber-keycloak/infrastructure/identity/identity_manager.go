...
func NewIdentityManager() *identityManager {
	return &identityManager{
		...
		restApiClientId:     viper.GetString("KEYCLOAK_RESTAPI_CLIENT_ID"),
		restApiClientSecret: viper.GetString("KEYCLOAK_RESTAPI_CLIENT_SECRET"),
	}
}
...
func (im *identityManager) CreateUser(
	ctx context.Context, user gocloak.User, password string, role string) (*gocloak.User, error) {
...
func (im *identityManager) RetrospectToken(ctx context.Context, accessToken string) (*gocloak.IntroSpectTokenResult, error) {
	...
	rptToken, err := client.RetrospectToken(ctx, accessToken, im.restApiClientId, im.restApiClientSecret, im.realm)
	if err != nil {
